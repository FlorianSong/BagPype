...
urllib.request.urlretrieve(
    ...
    "reduce_wwPDB_het_dict_download.txt",
)
...
for line in infile:
    if line.startswith("CONECT"):
        ...
        outlist = [
            x[0],
            "     ",
            make_pdb3_format_atom(x[1]),
            numconnections,
            " ".join(atomlist).rstrip(),
        ]
        ...
        line.startswith("HETNAM")
        or line.startswith("HETSYN")
        or line.startswith("FORMUL")
    ):
        outfile.write(line)
    else:
        print(line.strip(), file=outfile)
...
class record:
            ...
            if line.startswith("CONECT"):
                ...
                if parent_atom in hydrogens_to_remove:
                    self.lines[line_index] = (
                        "xx" + line
                    )  # assumes hydrogen always comes after oxygen
                elif (
                    parent_atom.startswith("O") and number_atoms == "2"
                ):  # need 2 connected atoms
    ...
    def process(self, formul_line, outfile):
        phosphate_pattern = re.compile(
            "(FORMUL)(.........)(.*H[1-9])(.*O[1-9])(.*P[1-9])"
        )
...
print("Removing phosphate hydorgens", file=sys.stderr)
...
if "reduce_wwPDB_het_dict.txt" in existing_files:
    print(
        "  The previous het dict was renamed to reduce_wwPDB_het_dict_obsoleted_"
        + fetchtime
        + ".txt",
        file=sys.stderr,
    )
    os.rename(
        ...
        "reduce_wwPDB_het_dict_obsoleted_" + fetchtime + ".txt",
    )
