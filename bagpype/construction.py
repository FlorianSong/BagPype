...
class Graph_constructor(object):
    ...
    def construct_graph(
        ...
        print(
            ...
            + " # atoms = %d, # bonds = %d" % (len(protein.atoms), len(protein.bonds))
        )
        print(
            (
                "    Time taken = %.2fs, final filename: %s"
                % (time2 - time1, protein.pdb_id)
            )
        )
        print()
    ...
    def _process_LINKs(self):
        ...
        for LINK_bond in LINK_bonds:
            ...
            if found_atoms[0] and found_atoms[1]:
                is_covalent = within_cov_bonding_distance(
                    found_atoms[0], found_atoms[1]
                )
                new_LINK_entry = (
                    found_atoms,
    ...
    def find_covalent_bonds(self):
        ...
        for atom1 in self.protein.atoms:
            for atom2 in self.protein.atoms[
                list(self.possible_bonds.neighbors(atom1.id))
            ]:
                if atom2.id > atom1.id:

                    if in_same_residue(atom1, atom2):

                        self.add_intra_residue_bond(
                            atom1, atom2, self.cov_bond_energies
                        )
                    else:
    ...
    def initialise_covbond_dictionary(self):
        ...
        dictionary = bagpype.energies.generate_energies_dictionary(
            list_of_present_residues
        )
    ...
    def add_inter_residue_bond(self, atom1, atom2):
        ...
        if atom1.element == "S" and atom2.element == "S":
            ...
            if bond_strength is not None:
                bond_strength *= self.k_factor / 6.022
                self.bonds.append(
                    bagpype.molecules.Bond([], atom1, atom2, bond_strength, "DISULFIDE")
                )
            ...
            if print_warnings:
                print(
                    (
                        ...
                        + atom2.res_name
                        + atom2.res_num
                        + " based on distance constraints using single bond energies!"
                    )
                )
            strength = bagpype.parameters.single_bond_energies[atom1.element][
                atom2.element
            ]
        else:
            strength = None
    ...
    def find_hydrogen_bonds(self):
        ...
        for hydrogen in [atom for atom in self.protein.atoms if atom.element == "H"]:
            ...
            for acceptor in [
                atom
                for atom in self.protein.atoms[
                    list(self.possible_bonds.neighbors(hydrogen.id))
                ]
                if atom.element in ["N", "O", "S"]
            ]:
                ...
                if (
                    ...
                    if d < self.donor_acceptor_cutoff and r < self.H_acceptor_cutoff:
                        ...
                        if (
                            self.donor_H_acceptor_min
                            <= np.rad2deg(theta)
                            <= self.donor_H_acceptor_max
                        ):
    ...
    def find_all_atom_triples(self, atom):
        ...
        if degree < 1:
            raise ValueError(
                "Couldn't find triplet for single atom {} with no bonds!".format(
                    str(atom.id)
                )
            )
        ...
        if atom.element == "N":
            ...
            if atom.res_name == "ASP" or atom.res_name == "GLU":
                ...
                if "CB" not in neighbour_atom_names:
                    sec_neighbour_ids = sec_neighborhood(
                        self.covalent_bonds_graph, atom.id
                    )
                    sec_neighbour_names = [
                        self.protein.atoms[a].name for a in sec_neighbour_ids
                    ]
    ...
    def determine_status_N(self, atom):
        ...
        if atom.res_name == "ASN" or atom.res_name == "GLN":
            ...
            if degree == 2:
                ...
                print(
                    "Something went wrong atom {0} is a Nitrogen".format(atom.id)
        ...
        for neighbour_atom in [
            ...
            if neighbour_atom.name == "CE1":
                ...
                for ce_neighbour_atom in [
                    ...
                    if (
                        ce_neighbour_atom.element == "N"
                        and ce_neighbour_atom.id != atom.id
                    ):
                        other_nitrogen = ce_neighbour_atom
                        for other_N_neighbour_atom in [
                            self.protein.atoms[k]
                            for k in self.covalent_bonds_graph.neighbors(
                                other_nitrogen.id
                            )
                        ]:
        ...
        if atom.res_name == "SER" or atom.res_name == "THR":
            ...
            if degree == 0:
                raise Exception(
                    "The Oxygen atom "
                    + str(atom.id)
                    + " has no covalent bonding partners."
                )
            ...
            if degree == 1:
                ...
                if any_neighbour_H:
                    return dict(
                        DonorAcceptor="both", Charged=False, Hybridisation="sp3"
                    )

                else:
                    return dict(
                        DonorAcceptor="acceptor", Charged=False, Hybridisation="sp3"
                    )

            elif degree > 2:
                print(
                    "Something went wrong. Atom {0} is an Oxygen ".format(atom.id)
    ...
    def determine_status_S(self, atom):
        ...
        if atom.res_name == "MET":
            ...
            if degree == 1:
                ...
                print(
                    "Something went wrong. Atom {0} is a Sulfur ".format(atom.id)
    ...
    def find_hydrophobics(self):
        ...
        total_hydrophobic_energy = sum(
            [hphobic_graph[i][j]["energy"] for i, j in matches]
        )
        print(
    ...
    def hydrophobic_selection(self, graph):
        ...
        for edge_not_in_mst in notmst.edges:
            ...
            weights_along_path = [
                graph[u][v]["energy"] for u, v in zip(path[:-1], path[1:])
            ]
    ...
    def hydrophobic_selection2(self, graph):
        ...
        # with open(self.protein.pdb_id[0:-4] + "_gammas.txt", "w") as f:
        #     for i, j in zip(x, y):
        #         atom_id1 = node_dictionary[i]
        #         atom_id2 = node_dictionary[j]
        #         gamma_val = gamma_matrix[i, j]
        #         mlink = LLink[i, j]
        #         didj = mD[i, j] * (2 / self.hydrophobic_RMST_gamma)
        ...
        #                 atom1.res_name,
        #                 atom2.res_name,
        #                 self.hydrophobic_potential(
        #                     distance_between_two_atoms(atom1, atom2),
        #                     atom1.element,
        #                     atom2.element,
        #                 ),
        #                 distance_between_two_atoms(atom1, atom2),
    ...
    def find_stacked(self):
        ...
        for pair_i in close_DNA_residues:
            ...
            base1 = base_key1[2]
            normal1 = np.cross(
                self.protein.atoms[id_list1[0]].xyz
                - self.protein.atoms[id_list1[1]].xyz,
                self.protein.atoms[id_list1[0]].xyz
                - self.protein.atoms[id_list1[2]].xyz,
            )
            ...
            base2 = base_key2[2]
            normal2 = np.cross(
                self.protein.atoms[id_list2[0]].xyz
                - self.protein.atoms[id_list2[1]].xyz,
                self.protein.atoms[id_list2[0]].xyz
                - self.protein.atoms[id_list2[2]].xyz,
            )
            ...
            for id1 in id_list1:
                q1, c1 = get_pisigma_charges(
                    base1,
                    normal1,
                    self.protein.atoms[id1].xyz,
                    self.protein.atoms[id1].name,
                )
        ...
        for key in sorted(
            ...
            for ring_atom in ["C2", "C4", "C5", "C6", "N1", "N3"]:
                ...
                atom1, atom2 = (
                    self.protein.atoms[atom1_id],
                    self.protein.atoms[atom2_id],
                )
        ...
        for i in range(len(OP1_atoms) - 1):
            for j in range(i, len(OP1_atoms)):
                ...
                if self.protein.atoms[atom1_id].chain == self.protein.atoms[
                    ...
                    rij = np.linalg.norm(
                        self.protein.atoms[atom1_id].xyz
                        - self.protein.atoms[atom2_id].xyz
                    )
                    energy = abs(
                        (-self.k_factor * 4.184 / 6.022)
                        * (332 / epsilon)
        ...
        if self.protein.LINKs:
            ...
            for LINK_entry in self._LINK_list:
                ...
                if LINK_entry[1]["distance"] != round(bond_length, 2):
                    print(
                        (
                            "WARNING: The LINK entry between {} and {} has a different distance value than the computed one (rounded to two decimal places):"
                            " Computed = {}, in PDB = {}. The computed one will be used."
                        ).format(atom1, atom2, bond_length, LINK_entry[1]["distance"])
                    )
                ...
                if LINK_entry[1]["is_covalent"]:
                    ...
                    try:
                        ...
                        bond_strength = (332 * charge1 * charge2) / (
                            epsilon * bond_length
                        )
                        bond_strength *= -self.k_factor * 4.184 / 6.022
                        if bond_strength > 0:
...
def within_cov_bonding_distance(atom1, atom2):
    ...
    return (
        ...
        and distance_between_two_atoms(atom1, atom2) < min(cutoff, 3.0)
        else False
    )
