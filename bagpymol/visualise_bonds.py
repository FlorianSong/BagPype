...
def visualise(
    ...
    list_of_types = (
        [x.upper() for x in list_of_types]
        if type(list_of_types) is list
        else [list_of_types.upper()]
    )
    print("Visualising: {} edges.".format(", ".join(list_of_types).lower()))

    colours = {
        "covalent": [187, 187, 187],  # BBBBBB
    ...
    for i, row in enumerate(reader):
        ...
        if len(specific_residues) > 0:
            current_residue1 = tuple(
                [str(bond_data["atom1_res_num"]), str(bond_data["atom1_chain"])]
            )
            current_residue2 = tuple(
                [str(bond_data["atom2_res_num"]), str(bond_data["atom2_chain"])]
            )
            print(current_residue1)
            if not any(
                [
                    current_residue1 in specific_residues,
                    current_residue2 in specific_residues,
                ]
            ):
                continue
        ...
        if len(bond_type) == 1:
            ...
            if bond_type[0] in list_of_types:
                ...
                cmd.distance(prefix + bond_id, "id " + atom1_id, "id " + atom2_id)
        ...
        if bond_type[0] in ["COVALENT", "DISULFIDE"] and bond_type[0] in list_of_types:
            ...
            bond_type[0] in ["HYDROGEN", "SALTBRIDGE"] and bond_type[0] in list_of_types
        ):
            cmd.set("dash_gap", 0, prefix + bond_id)
